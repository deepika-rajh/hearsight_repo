...
static void writePLYPointCloud(const std::string &ply_file_path, const rv_dfs::PointCloudType<float>& pointCloud)
{
...
static rvStereoCamera importStereoCalData(const std::string& file) //Translation should be millimeter
{
    ...
    param.camera[0].principalPoint[0] = (float32_t) camera_mat_left.at<double>(0, 2);
    param.camera[0].principalPoint[1] = (float32_t) camera_mat_left.at<double>(1, 2);
    ...
    param.camera[1].principalPoint[0] = (float32_t) camera_mat_right.at<double>(0, 2);
    param.camera[1].principalPoint[1] = (float32_t) camera_mat_right.at<double>(1, 2);
    ...
    param.camera[0].pixelStride = image_size[0];
    param.camera[1].pixelWidth = image_size[0];
    param.camera[1].pixelHeight = image_size[1];
    param.camera[1].pixelStride = image_size[0];
    ...
    {
        param.camera[0].distortionModel = FisheyeModel4;
        param.camera[1].distortionModel = FisheyeModel4;
        for (int i = 0; i < 4; ++i)
        {
            if (distortion_left.rows > 1)
                param.camera[0].distortion[i] = (float32_t) distortion_left.at<double>(i, 0);
            else
                param.camera[0].distortion[i] = (float32_t) distortion_left.at<double>(0, i);
            if (distortion_right.rows > 1)
                param.camera[1].distortion[i] = (float32_t) distortion_right.at<double>(i, 0);
            else
                param.camera[1].distortion[i] = (float32_t) distortion_right.at<double>(0, i);
        }
        for (int i = 4; i < 8; ++i)
        {
    ...
    {
        param.camera[0].distortionModel = RationalModel8;
        param.camera[1].distortionModel = RationalModel8;
        for (int i = 0; i < 8; ++i)
        {
            if (distortion_left.rows > 1)
                param.camera[0].distortion[i] = (float32_t) distortion_left.at<double>(i, 0);
    ...
    {
        param.camera[0].distortionModel = Polynomial5;
        param.camera[1].distortionModel = Polynomial5;
        for (int i = 0; i < 5; ++i)
        {
            if (distortion_left.rows > 1)
                param.camera[0].distortion[i] = (float32_t) distortion_left.at<double>(i, 0);
...
DFSRosTwoNode::DFSRosTwoNode(const rmw_qos_profile_t& qos): Node("dfs_ros2_node"), init_flag(false), color_flag(false), calib_file_flag(false), stitch_img_flag(false), color_point_flag(false), get_stereo(false), get_color(false)
{
    np = std::shared_ptr<rclcpp::Node>(this);
    qos_prf = qos;

    disp_pub = image_transport::create_publisher(this, "/dfs_ros2/disparity_image", qos);
    dep_pub = image_transport::create_publisher(this, "/dfs_ros2/depth_raw", qos);  // short 16bit, mm unit. same to RealSense.
    left_pub = image_transport::create_publisher(this, "/dfs_ros2/left_image", qos);
    point_pub =  this->create_publisher<sensor_msgs::msg::PointCloud2>("/dfs_ros2/point_cloud", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw( qos ), qos) );
    rectl_pub = image_transport::create_publisher(this, "/dfs_ros2/left_rect", qos);
    rectr_pub = image_transport::create_publisher(this, "/dfs_ros2/right_rect", qos);
}
...
void DFSRosTwoNode::init_dfs()
{
    ...
    if( disp_lvl >256) disp_lvl=256;

    if( calib_file_flag) // read from file
    {
        stereo_param = importStereoCalData(calib_file_path);
        if (stereo_param.camera[0].pixelWidth  == 0){ std::cout<< "Not right stereo calibration file error." << std::endl; return ; }
    ...
    else
    {
        ...
        rclcpp::wait_for_message<sensor_msgs::msg::CameraInfo>(cm1, np, "/camera/infra1/camera_info");  // must rclcpp::Node::SharedPtr
        rclcpp::wait_for_message<sensor_msgs::msg::CameraInfo>(cm2, np, "/camera/infra2/camera_info");
        parseCameraInfo( cm1, cm2);
    }

    if( stitch_img_flag) stride = 2 * width;
    debug_info("first");
    pixel = width * height;
    ...
    if( !init_flag){ std::cout<< "Not appropriate params. Init failed." << std::endl; return ; }
    ...
    in.version = 0x00010000;
    in.input.imgLeft = nullptr;
    in.input.imgRight = nullptr;

    in.numParams = 1;
    in.paramSize = sizeof(rvDFS_InputParam_V1_t);  // not zero to enable postprocessing.
    // in.dfsParam = nullptr; // not set roi.
    in.dfsParam = &dfs_run_param;
    in.dfsParam->roi = new rvRoi2D();
    in.dfsParam->roi->x = 0;
    in.dfsParam->roi->y = 0;
    in.dfsParam->roi->width = width;
    in.dfsParam->roi->height = height;

    out.version = 1;
    out.roi = *(in.dfsParam->roi);
    out.dim.width = width;
    out.dim.height = height;
    out.imgL = nullptr; // hdr image
    out.imgR = nullptr;
    out.mapDataType = 0; // 0 float, 1 int16. 

    out.rectL = rectl.data;
    out.rectR = rectr.data;
    out.mapOfDisparity = dispImg.data;
    out.mapOfDepth = depImg.data;
    out.pointBuffer = &pc;
    out.numPoints = pixel;
    ...
    if( stitch_img_flag)
    {
        stitch_sub = this->create_subscription<sensor_msgs::msg::Image>("/camera/image", 3, std::bind(&DFSRosTwoNode::callbackStitchImg, this, _1));
        t_stereo = std::make_shared<std::thread>(&DFSRosTwoNode::processStitchImg, this);
    }
    else // parse img1 and img2
    {
        ...
        sy->registerCallback( std::bind(&DFSRosTwoNode::callbackTwoImgs, this, _1, _2));
        t_stereo = std::make_shared<std::thread>(&DFSRosTwoNode::processTwoImgs, this);
    }

    if( color_flag)
    {
        color_pub = image_transport::create_publisher(this, "/dfs_ros2/color_image", qos_prf);
        color_sub = this->create_subscription<sensor_msgs::msg::Image>("/camera/color/image_raw", 10, std::bind(&DFSRosTwoNode::callbackColor, this, _1)); // must not 1. 1 will cause callbackColor dead or stuck status.
...
{
    if( dfs_base!=nullptr) dfs_base->deInitialize();
    dfs_base = nullptr;

    con_stereo.notify_one();
    if( t_stereo && t_stereo->joinable()) t_stereo->join();

    if( color_flag)
    {
        con_color.notify_one();
        if( t_color && t_color->joinable()) t_color->join();
    }
}

void DFSRosTwoNode::parseCameraInfo(const sensor_msgs::msg::CameraInfo& cm1, const sensor_msgs::msg::CameraInfo& cm2)
{
    height = cm1.height;
    width = cm1.width;
    stride = width;
...
void DFSRosTwoNode::processColor()
{
    while( rclcpp::ok()){
        ...
        sensor_msgs::msg::Image::SharedPtr one_msg = cv_bridge::CvImage(head_one, enc::BGR8, one).toImageMsg();
        color_pub.publish( *one_msg);
    }
}

void DFSRosTwoNode::callbackColor(const sensor_msgs::msg::Image::SharedPtr color_p) // must no & reference.
{
    mtx_color.lock();
...
void DFSRosTwoNode::callbackTwoImgs(const sensor_msgs::msg::Image::ConstSharedPtr& img_p1, const sensor_msgs::msg::Image::ConstSharedPtr& img_p2)
{
    mtx_stereo.lock();
    cv_p1 = cv_bridge::toCvCopy(img_p1, enc::MONO8); // alloc mem and copy data to cv_p1
...
void DFSRosTwoNode::processTwoImgs()
{
    while( rclcpp::ok()){
        ...
        in.input.imgLeft = now_img1.ptr<uint8_t>();
        in.input.imgRight = now_img2.ptr<uint8_t>();
        ...
        sensor_msgs::msg::Image::SharedPtr left_msg = cv_bridge::CvImage(genHeader(left_id), enc::MONO8, now_img1).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr disp_msg = cv_bridge::CvImage(genHeader(disp_id), enc::BGR8, dispColor).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr dep_msg = cv_bridge::CvImage(genHeader(dep_id), enc::TYPE_16UC1, depShort).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr rectl_msg = cv_bridge::CvImage(genHeader(rectl_id), enc::MONO8, rectl).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr rectr_msg = cv_bridge::CvImage(genHeader(rectr_id), enc::MONO8, rectr).toImageMsg();
        ...
        elapsed = stop - start;
        std::cout << "[INFO] Time(ms) of calcauteAll is: " << elapsed.count() * 1000 << std::endl;
    }
}

void DFSRosTwoNode::callbackStitchImg(const sensor_msgs::msg::Image::SharedPtr img_p1)
{
    mtx_stereo.lock();
    cv_p1 = cv_bridge::toCvCopy(img_p1, enc::MONO8); // alloc mem and copy data to cv_p1
    get_stereo = true;
...
void DFSRosTwoNode::processStitchImg()
{
    ...
    while( rclcpp::ok()){
        ...
        in.input.imgLeft = now_img1.ptr<uint8_t>(); // left-and-right img
        dfs_base->compute(&in, &out);
        ...
        toShort16(depImg, depShort);
        // if( dfs_cnt<3) debug_img();
        sensor_msgs::msg::Image::SharedPtr left_msg = cv_bridge::CvImage(genHeader(left_id), enc::MONO8, left_img).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr disp_msg = cv_bridge::CvImage(genHeader(disp_id), enc::BGR8, dispColor).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr dep_msg = cv_bridge::CvImage(genHeader(dep_id), enc::TYPE_16UC1, depShort).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr rectl_msg = cv_bridge::CvImage(genHeader(rectl_id), enc::MONO8, rectl).toImageMsg();
        sensor_msgs::msg::Image::SharedPtr rectr_msg = cv_bridge::CvImage(genHeader(rectr_id), enc::MONO8, rectr).toImageMsg();
...
// sensor_msgs::msg::PointCloud2 DFSRosTwoNode::genPointCloudMsg(const std::string& frame_id)
// {
...
// sensor_msgs::msg::PointCloud2 DFSRosTwoNode::genPointCloudColorMsg(const std::string& frame_id)
// {
//     sensor_msgs::msg::PointCloud2 cloud;
//     cloud_msg.header = genHeader(frame_id);
//     cloud_msg.height = 1;
...
void DFSRosTwoNode::updateCloudMsg()
{
    ...
    if( color_point_flag)  // xyzrgb
    {
        ...
        if( rectl.channels()==1 ){  // xyzggg
            for(int i=0; i<pc.size(); i++){
                ii = width * (int)(cy - pc[i][2]* fy / pc[i][0]) + (int)(cx - pc[i][1] * fx/ pc[i][0] + 0.5f); // hwc
                offset = i* cloud_msg.point_step;
            ...
            for(int i=0; i<pc.size(); i++){
                ii =3 * (width * (int)(cy - pc[i][2]* fy / pc[i][0]) + (int)(cx - pc[i][1] * fx/ pc[i][0] + 0.5f) ); // hwc
                offset = i* cloud_msg.point_step;
                memcpy(&cloud_msg.data[offset], pc[i].data(), 12);
                memset(&cloud_msg.data[offset + 12], 0, 4);
                cloud_msg.data[offset + 16] = p1[ii];
...
void DFSRosTwoNode::debug_info(const std::string& s)
{
    ...
    std::cout << "translation param: " << param.translation[0] << "," << param.translation[1] << "," << param.translation[2] << std::endl;
    std::cout << "rotation param: " << param.rotation[0] << "," << param.rotation[1] << "," << param.rotation[2] << std::endl;

    std::cout << "camera 1 param: " << std::endl;
    std::cout << "camera width, height, stride: " << param.camera[0].pixelWidth << "," << param.camera[0].pixelHeight << ", "<< param.camera[0].pixelStride<< std::endl;
    std::cout << "focal length: " << param.camera[0].focalLength[0] << "," << param.camera[0].focalLength[1] << std::endl;
    std::cout << "principal point: " << param.camera[0].principalPoint[0] << "," << param.camera[0].principalPoint[1] << std::endl;
    std::cout << "distortionModel: " << param.camera[0].distortionModel << std::endl;
    std::cout << "distortion:  " << param.camera[0].distortion[0] << ", " << param.camera[0].distortion[1];
    std::cout << "," << param.camera[0].distortion[2] << ", "<< param.camera[0].distortion[3];
    std::cout << "," << param.camera[0].distortion[4] << ", "<< param.camera[0].distortion[5];
    std::cout << "," << param.camera[0].distortion[6] << ", "<< param.camera[0].distortion[7];
    std::cout << std::endl;

    std::cout << "camera 2 param: " << std::endl;
    std::cout << "camera width, height, stride: " << param.camera[1].pixelWidth << "," << param.camera[1].pixelHeight << ", "<< param.camera[1].pixelStride<< std::endl;
    std::cout << "focal length: " << param.camera[1].focalLength[0] << "," << param.camera[1].focalLength[1] << std::endl;
    std::cout << "principal point: " << param.camera[1].principalPoint[0] << "," << param.camera[1].principalPoint[1] << std::endl;
    std::cout << "distortionModel: " << param.camera[1].distortionModel << std::endl;
    std::cout << "distortion:  " << param.camera[1].distortion[0] << ", " << param.camera[1].distortion[1];
    std::cout << "," << param.camera[1].distortion[2] << ", "<< param.camera[1].distortion[3];
    std::cout << "," << param.camera[1].distortion[4] << ", "<< param.camera[1].distortion[5];
    std::cout << "," << param.camera[1].distortion[6] << ", "<< param.camera[1].distortion[7];
    std::cout << std::endl;
    std::cout << "--------------------------------------------------" << std::endl;
...
void DFSRosTwoNode::debug_img()
{
    ...
    if( !now_img1.empty()) cv::imwrite(dbg_dir + "/left_" + sfx, now_img1);
    if( !now_img2.empty()) cv::imwrite(dbg_dir + "/right_" + sfx, now_img2);
    if( !dispColor.empty()) cv::imwrite(dbg_dir + "/disp_" + sfx, dispColor);
    if( !depShort.empty()) cv::imwrite(dbg_dir + "/dep_" + sfx, depShort);
    if( !dispImg.empty()) cv::imwrite(dbg_dir + "/dispRaw_" + sfx, dispImg);

    if( pc.size()) writePLYPointCloud(dbg_dir + "/pc_"+ pc_sfx, pc);
}
...
int main(int argc, char** argv)
{
    ...
    std::shared_ptr<DFSRosTwoNode> cnode = std::dynamic_pointer_cast<DFSRosTwoNode>(gnode);
    cnode->init_dfs();
