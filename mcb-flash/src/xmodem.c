...
bool xmodem_send(int fd, char * bin_arr, size_t bin_size, int * p_count)
{
  ...
  while (bin_len > 0) {
    ...
    mcb_qrc_write(fd, frame, sizeof(frame));
    printf("send frame %d \n", chunk);
    ...
    while (try--) {
      usleep(20000);
      while (!mcb_qrc_read(fd, &rec_ack, 1))
        ;
  ...
  // send EOF.
  mcb_qrc_write(fd, &end_eot, sizeof(uint8_t));
  usleep(50000);
  if (mcb_qrc_read(fd, &rec_ack, 1)) {
    /* check if recevice  ACK */
    if (rec_ack == X_ACK)
      printf("bootloader send frame done\n");
    else {
      mcb_qrc_write(fd, &end_eot, sizeof(uint8_t));
      printf("send again EOF\n");
    }
  }
  return true;
}
