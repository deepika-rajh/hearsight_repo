...
*******************************************************************************/
#ifndef _INPUT_RGBD_CAMERA_ROS2_H_
#define _INPUT_RGBD_CAMERA_ROS2_H_
...
typedef message_filters::Synchronizer<message_filters::sync_policies::ApproximateTime<sensor_msgs::msg::Image, sensor_msgs::msg::Image>>  RGBDSync;

class InputRGBDCameraROS2: public CameraInterface
{
public:
    using CallbackFunc = std::function<void(const int64_t timestamp, const uint8_t * imageBuf, const uint16_t * depthBuf)>;

    InputRGBDCameraROS2(rclcpp::Node::SharedPtr const & node_, const std::string & config);
    ~InputRGBDCameraROS2();
...
private:
    ...
    static rclcpp::Subscription<sensor_msgs::msg::CameraInfo>::SharedPtr depthInfoSub;

    std::shared_ptr<RGBDSync> syncApproximate;
