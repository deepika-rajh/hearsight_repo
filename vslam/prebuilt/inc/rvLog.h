...
inline void __declspec(dllexport)
    RVDebugLog(enum AppLoglevel level, const char *format, ...) {
  if (RV_LOG_LEVEL < (int)level)
    return;
  va_list args;
  va_start(args, format);
  vfprintf(RV_CONSOLE_STREAM, format, args);
  fflush(RV_CONSOLE_STREAM);
  va_end(args);
...
#define RV_ERR(fmt, ...)                                                       \
  do {                                                                         \
    ...
    RVDebugLog(RV_LOG_ERROR, "%d:%d:%d.%d %s:%d ERROR: " fmt "\n", h, m, s,    \
               ms, __FILE__, __LINE__, __VA_ARGS__);                           \
...
#define RV_ERR(fmt, ...)                                                       \
  do {                                                                         \
    if (RV_LOG_LEVEL >= RV_LOG_ERROR) {                                        \
      ...
      RV_TS(sec, usec);                                                        \
      syslog(LOG_ALERT, "(%d.%06d) %s:%d ERROR: " fmt, sec, usec, __FILE__,    \
...
#define RV_INFO(fmt, ...)                                                      \
  do {                                                                         \
    if (RV_LOG_LEVEL >= RV_LOG_INFO) {                                         \
      unsigned int sec, usec;                                                  \
      RV_TS(sec, usec);                                                        \
      syslog(LOG_ALERT, "(%d.%06d) %s:%d INFO: " fmt, sec, usec, __FILE__,     \
             __LINE__, ##__VA_ARGS__);                                         \
      if (RV_STDERR_LOGGING) {                                                 \
...
#define RV_DBG(fmt, ...)                                                       \
  do {                                                                         \
    if (RV_LOG_LEVEL >= RV_LOG_DEBUG) {                                        \
      unsigned int sec, usec;                                                  \
      RV_TS(sec, usec);                                                        \
      syslog(LOG_ALERT, "(%d.%06d) %s:%d DEBUG: " fmt, sec, usec, __FILE__,    \
             __LINE__, ##__VA_ARGS__);                                         \
      if (RV_STDERR_LOGGING) {                                                 \
