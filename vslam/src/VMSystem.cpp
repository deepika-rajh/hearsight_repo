...
std::shared_ptr<VMSystem> VMSystem::Initialize(const std::string& algSetting, std::shared_ptr<CameraInterface> camera)
{
...
void VMSystem::addDepthImage(const int64_t timestamp, const uint8_t* imageBuf, const uint16_t* depthBufInput)
{
    ...
    if (rvVM_GetGridMap(vmPtr, &gridMap, &gridWidth, &gridHeight, &gridOriginX, &gridOriginY, &gridTimestamp))
    {
        ...
        delete[]gridMap;
        gridMap = NULL;
        showOccupancyImg(gridImage);
    }
    if (rvVM_GetDepthImage(vmPtr, depthImage))
    {
        cv::Mat depthImageMat(height, width, CV_16UC1, depthImage);
        double min;
        double max;
        cv::minMaxIdx(depthImageMat, &min, &max);
        double scale = 255. / (max - min);
        depthImageMat.convertTo(depthImageMat, CV_8UC1, scale, -min * scale);
        cv::Mat colorsMap;
        cv::applyColorMap(depthImageMat, colorsMap, cv::COLORMAP_JET);
        showDepthImage(colorsMap);        
    }
