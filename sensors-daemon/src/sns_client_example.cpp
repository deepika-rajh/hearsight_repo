...
static get_raw_data_func_t get_raw_func = NULL;
/**
  ...
  for(int i = 0; i < pb_event_msg.events_size(); i++)
  {
    ...
    else if(SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT == pb_event.msg_id())
    {
      ...
      printf("Received sample <%f, %f, %f>\r\n",
          event.data(0), event.data(1), event.data(2));
      if (get_raw_func != NULL)
      {
        get_raw_func(event.data(0), event.data(1), event.data(2));
      }
      total_samples_rxved++;
    }
    else
...
int register_data_callback(get_raw_data_func_t data_func)
{
  if (data_func == NULL)
  {
    return -1;
