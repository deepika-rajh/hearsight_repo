...
bool GetCameraParameter( const char *cameraID, rvCameraParams & configuration )
{
   ...
   while( std::getline( cfg, line ) )
   {
      ...
      iss >> itemName;
      printf("print item name: %s \n",itemName.c_str());
      if( itemName.compare( "image_width:" ) == 0 )
      {
         iss >> configuration.stereo.camera[0].pixelWidth;
      }
      else if( itemName.compare( "image_height:" ) == 0 )
      {
         iss >> configuration.stereo.camera[0].pixelHeight;
      }
      else if( itemName.compare( "camera_matrix:" ) == 0 )
      {
		 float cMatrix[9];
         ReadMatrix( cfg, cMatrix );
		 configuration.stereo.camera[0].principalPoint[0] = cMatrix[2];
		 configuration.stereo.camera[0].principalPoint[1] = cMatrix[5];
		 configuration.stereo.camera[0].focalLength[0] = cMatrix[0];
		 configuration.stereo.camera[0].focalLength[1] = cMatrix[4];
      }
      else if( itemName.compare( "distortion_coefficients:" ) == 0 )
      {
         memset( configuration.stereo.camera[0].distortion, 0, sizeof( configuration.stereo.camera[0].distortion ) );
         ReadMatrix( cfg, configuration.stereo.camera[0].distortion );
         printf("print distortion_coefficients %f, %f, %f, %f, %f, %f, %f, %f \n",configuration.stereo.camera[0].distortion[0],
                configuration.stereo.camera[0].distortion[1],
                configuration.stereo.camera[0].distortion[2],
                configuration.stereo.camera[0].distortion[3],
                configuration.stereo.camera[0].distortion[4],
                configuration.stereo.camera[0].distortion[5],
                configuration.stereo.camera[0].distortion[6],
                configuration.stereo.camera[0].distortion[7]);
      }
      else if( itemName.compare( "distortion_model:" ) == 0 )
      {
         ...
         {
            configuration.stereo.camera[0].distortionModel = rvDistortionModel::FisheyeModel4;
         }
         else
         {
            configuration.stereo.camera[0].distortionModel = rvDistortionModel::RationalModel8;
         }
      }
      ...
      else if( itemName.compare( "projection_matrix:" ) == 0 )
      {
		 ...
		 for (int i =0, k=0; i<3; i++)
		     for(int j=0; j<4; j++,k++)
             {
                 configuration.stereoRect.camera[0].P[i][j] = p[k];
                 printf("print projection_matrix: %f \n",p[k]);
      ...
      {
         iss >> configuration.stereoRect.camera[0].pixelWidth;
         printf("project_image_width %d\n", configuration.stereoRect.camera[0].pixelWidth);

      }
      else if( itemName.compare( "project_image_height:" ) == 0 )
      {
         iss >> configuration.stereoRect.camera[0].pixelHeight;
          printf("project_image_height %d\n",configuration.stereoRect.camera[0].pixelHeight);
      }
   }
...
bool InputCamera_OV9282::ParsePlaybackParameters( const char *configFile )
{
   ...
   printf( "\n****** Parsing paths......\n" );
   std::string fullName =  configFile; //Program_Root +
   std::ifstream cfg( fullName, std::ifstream::in );
