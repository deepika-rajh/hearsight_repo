...
void MapFocuser::AddPoseInWorld( mvVSLAMTrackingPose& pose )
{
   ...
   /*Lei: also update visualization range*/
   m_xMinKF = m_xMinKF < pose.pose.matrix[0][3] ? m_xMinKF : pose.pose.matrix[0][3];
   m_xMaxKF = m_xMaxKF > pose.pose.matrix[0][3] ? m_xMaxKF : pose.pose.matrix[0][3];
   m_yMinKF = m_yMinKF < pose.pose.matrix[1][3] ? m_yMinKF : pose.pose.matrix[1][3];
   m_yMaxKF = m_yMaxKF > pose.pose.matrix[1][3] ? m_yMaxKF : pose.pose.matrix[1][3];
}
...
std::set<int> & MapFocuserBase::GetKFsIdToDeactivate()
{
   return m_farawayKFs;
}


std::set<int> & MapFocuserBase::GetKFsIdToRemove()
{
   return m_removedKFs;
}

const std::vector<MV_ActiveKeyframe> & MapFocuserBase::getActiveKeyframe() const
{
   return m_activeKeyframes;
}


std::set<int> & MapFocuserBase::GetKFsIdToLoad()
{
   return m_kernalKFs;
}
