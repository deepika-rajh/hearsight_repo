...
class MapFocuser: public MapFocuserBase
{
   friend void DetermineVisualizationSize( const MV_ActiveKeyframe * keyframes, const uint32_t keyframeNum, cv::Mat & );
...
{
public:
   void AddPoseInWorld( mvVSLAMTrackingPose & )
   {}
   void SetCrossCalibrationMatrix( const float*)
   {}
   void AddScalessPose( mvVSLAMTrackingPose& )
   {}
   bool TransformationReady() const
   {
      return true;
   }
   void UpdateKeyframeState( MV_ActiveKeyframe* , int, bool )
   {}  
   bool NeedSpatialInWorld() const
   {
      return true;
   ...
   void SetSpatialInWorldMatrix( const float* )
   {  }
