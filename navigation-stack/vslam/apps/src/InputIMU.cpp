...
{
   const char *desc = "INV-ICM20602";
   size_t desc_len = strnlen(desc, SENSOR_IMU_MAX_DESCRIPTION_LEN);
   if(desc_len == SENSOR_IMU_MAX_DESCRIPTION_LEN)
	   desc_len = SENSOR_IMU_MAX_DESCRIPTION_LEN - 1;

   imu_id_array[0].imu_id = 1;
   //strlcpy(imu_id_array[0].imu_description, desc, desc_len); //unused for RB5
   imu_id_array[0].imu_description[desc_len] = '\0';
...
)
{
   if(SENSOR_CLOCK_SYNC_TYPE_REALTIME == type)
   {
      ...
      //strlcpy (address.sun_path, "/data/imud_socket", sizeof(address.sun_path);
      len = sizeof (address);
